...
func _ready():
	...
	get_tree().get_root().get_node("Main/ScreenWrapperAreas/WrapArea").connect(
		"body_entered", self, "_on_WrapArea_body_entered", [], Object.CONNECT_ONESHOT
	)
	get_tree().get_root().get_node("Main/ScreenWrapperAreas/PlayArea").connect(
		"body_exited", self, "_on_PlayArea_body_exited", [], Object.CONNECT_ONESHOT
	)
